...
export default function App() {
  ...
  return (
    <div style={{ background: T.cream, minHeight: "100vh", fontFamily: "'DM Sans','Segoe UI',sans-serif", maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column" }}>
      ...
      {tab === "agent" && biz?.id && <OutreachAgent bizId={biz.id} bizName={biz.name} bizEmail={biz.email}/>}
      {tab === "training" && <TrainingCompliance staff={staff} bizName={biz?.name}/>}
      {tab === "pipeline" && biz?.id && <HotLeads bizId={biz.id} bizName={biz.name}/>}
      {tab === "inbox" && <RepliesInbox bizName={biz?.name} bizPhone={biz?.phone} bizDirector={biz?.directorName || "Director"}/>}
      {tab === "more" && <div style={{fontFamily:"system-ui,sans-serif",background:"#f5f5f5",minHeight:"100vh",paddingBottom:100}}><div style={{background:"#0A1F44",padding:"52px 16px 16px"}}><div style={{fontSize:11,color:"#C9A84C",textTransform:"uppercase",letterSpacing:2,fontWeight:700,marginBottom:4}}>{biz?.name || "My Business"}</div><div style={{fontSize:20,fontWeight:800,color:"#fff"}}>More</div></div><div style={{padding:16,display:"flex",flexDirection:"column",gap:10}}>{[{id:"inbox",l:"Inbox",i:"📬"},{id:"pipeline",l:"Pipeline",i:"🔥"},{id:"training",l:"Training",i:"📋"},{id:"invoices",l:"Invoices",i:"💰"},{id:"staff",l:"Staff",i:"👷"},{id:"expenses",l:"Expenses",i:"🧾"},{id:"reports",l:"Reports",i:"📊"},{id:"reviews",l:"Reviews",i:"⭐"},{id:"portal",l:"Client Portal",i:"🔐"},{id:"settings",l:"Settings",i:"⚙️"}].map(t=>(<button key={t.id} onClick={()=>t.id==="portal"?setClientPortal(true):setTab(t.id)} style={{display:"flex",alignItems:"center",gap:14,background:"#fff",border:"none",borderRadius:14,padding:"16px",cursor:"pointer",boxShadow:"0 2px 6px rgba(0,0,0,0.05)",textAlign:"left"}}><span style={{fontSize:24}}>{t.i}</span><span style={{fontSize:15,fontWeight:600,color:"#0A1F44"}}>{t.l}</span><span style={{marginLeft:"auto",color:"#ccc"}}>›</span></button>))}<button onClick={handleLogout} style={{display:"flex",alignItems:"center",gap:14,background:"#fff",border:"1px solid #ffeeee",borderRadius:14,padding:"16px",cursor:"pointer",textAlign:"left",marginTop:8}}><span style={{fontSize:24}}>🚪</span><span style={{fontSize:15,fontWeight:600,color:"#e74c3c"}}>Log Out</span></button></div></div>}
      {tab === "dashboard" && <><div style={{background:"red",color:"white",fontSize:10,padding:4,textAlign:"center"}}>BIZ ID: {biz?.id}</div><Dashboard biz={biz} clients={clients} jobs={jobs} invoices={invoices} expenses={expenses} notifications={notifications} setTab={setTab} /></>}
